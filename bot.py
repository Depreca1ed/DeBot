    ...
    async def blacklistcheck(self, ctx: commands.Context[Self]) -> bool:
        ...
        if isinstance(ctx.channel, discord.channel.DMChannel):
            await ctx.channel.send(
                f"Hey {str(ctx.author)}! You are currently blacklisted from using the bot. You will not be able to run my commands in any servers or DMs"
            )
        return False
    ...
    async def setup_hook(self) -> None:
        ...
        for cog in [m.name for m in iter_modules(["cogs"], prefix="cogs.")]:
            try:
                ...
                log.error(f"Ignoring exception in loading {cog}", exc_info=error)
            else:
