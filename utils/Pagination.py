...
class ElyPagination(discord.ui.View):
    ...
    def __init__(self, embeds: List[ElyEmbed], ctx: commands.Context[Elysian], *, has_timeout: Optional[bool] = False):
    ...
    async def first_page(self, inter: discord.Interaction[Elysian], _: discord.ui.Button[discord.ui.View]):
        self.index = 0
        self._update_state()
        await inter.response.edit_message(embed=self.embeds[self.index], view=self)

    @discord.ui.button(emoji="<:emoji_6:1238879362628059196>", style=discord.ButtonStyle.secondary)
    async def prev_page(self, inter: discord.Interaction[Elysian], _: discord.ui.Button[Self]):
        self.index -= 1
        self._update_state()
        await inter.response.edit_message(embed=self.embeds[self.index], view=self)

    @discord.ui.button(emoji="<:emoji_9:1238879529662025748>", style=discord.ButtonStyle.grey)
    async def remove(self, inter: discord.Interaction[Elysian], _: discord.ui.Button[Self]):
    ...
    async def next_page(self, inter: discord.Interaction[Elysian], _: discord.ui.Button[Self]):
        self.index += 1
        self._update_state()
        await inter.response.edit_message(embed=self.embeds[self.index], view=self)

    @discord.ui.button(emoji="<:emoji_9:1238879493750394931>", style=discord.ButtonStyle.grey)
    async def last_page(self, inter: discord.Interaction[Elysian], _: discord.ui.Button[Self]):
